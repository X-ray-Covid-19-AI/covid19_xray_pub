...
from pre_proc.Demo import mygen
from pre_proc.mask_creator import MaskCreator
IMAGE_SHAPE = (256, 256)
...
def mask_generator(csv_path,outfolder,path,output_size):
    """
    creates mask for given list of files
    :param csv_path: full path to csv file that contains image filenames
    :type csv_path: str
    :param outfolder: folder that contains logfile
    :type outfolder: str
    :param path: directory prefix to filenames in csv_path
    :type path: str
    :param output_size: size of desired mask
    ...
    for original, xx in composite_generator:
        ...
        yield pred
from skimage import  img_as_float,transform,io
...
def get_mask(image):
    ...
    model = mask_creator.get_model()
    mask = model.predict(image)[..., 0].reshape(IMAGE_SHAPE)
